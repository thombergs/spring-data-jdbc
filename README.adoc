...
R2DBC is the abbreviation for https://github.com/r2dbc/[Reactive Relational Database Connectivity], an experiment to integrate relational databases using a reactive driver.

The state of R2DBC is experimental to evaluate how an reactive integration could look like. To get started, you need a R2DBC driver first. Currently, https://github.com/r2dbc/r2dbc-postgresql[PostgreSQL] is the only available driver. All functionality is encapsulated in `DatabaseClient`:
